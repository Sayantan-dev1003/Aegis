...
3. **SLA breach logic is now symmetric.** In v1, a reviewer who *proactively rejected* a case (honest behavior) got *less* time in the Default Queue, while a reviewer who silently let the SLA expire (negligent behavior) got a *full fresh* SLA. This rewards negligence over honesty. **Fix:** reject → full fresh SLA. Silent breach → reduced SLA + logged as a negligence incident against that reviewer.

4. **The reject loop is capped.** In v1, nothing stopped a case from bouncing between queues indefinitely if every reviewer rejected it. **Fix:** one reject is allowed; a second forces mandatory claim by Admin, no further reject option.
...
2. **Synchronous rule evaluation.** In the same DB transaction as the write, the rules engine evaluates all active rules against this transaction. This produces one action: `block`, `flag`, or nothing.
...
- 16 transactions match a `flag` rule → `escalated`, routed by priority (e.g. a transaction matching both "VIP Ultra-High Value" and "Structuring Pattern" goes to VIP / White-Glove Support, since VIP is Priority 1). This now includes transactions matching "Account Takeover (ATO) Prevention" and "Shared Network Protection", which are routed directly to the Account Takeover Suspects queue as `flag` actions.
...
- Of the 16 rule-`escalated`: each gets a `risk_score` written. Reviewers now see their worklist sorted highest-risk-first instead of FIFO.
...
End state after processing: 70 auto-approved, 8 escalated to ML Borderline Review (6 pending-turned-borderline + 2 ML-triggered blocks logged separately), 6 auto-blocked (1 flagged for audit), 16 in rule-based queues (now risk-sorted). Every single one of the 100 has a risk score attached somewhere in the system — nothing is invisible to ML anymore.
...
### Case 8 — ATO Prevention rule triggers, direct escalation
A transaction trips "Account Takeover (ATO) Prevention" (`velocity >= 7`) → `status = escalated`, immediately routed to the Account Takeover Suspects queue with HIGH priority. No customer-facing friction — the reviewer handles it directly. In parallel, ML scores the transaction and enriches the row, so the reviewer sees the risk score alongside the velocity trigger reason.
...
- "Account Takeover (ATO) Prevention" and "Shared Network Protection" now use `action = flag` targeting "Account Takeover Suspects". This is a behaviour-preserving simplification — the outcome (human review of suspected ATO transactions) is identical, without the complexity of a customer-facing challenge flow.
...
Rules decide **what happens right now** (block / flag / pass). ML decides **how urgently a human should care**, and it gets a vote on every transaction, all the time — not just the ones rules didn't already touch.

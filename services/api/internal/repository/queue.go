...
func (r *QueueRepository) List(ctx context.Context) ([]model.Queue, error) {
	updateQuery := `
		UPDATE queues q SET
			open_cases = (SELECT COUNT(DISTINCT t.id) FROM transactions t WHERE t.ingested_at >= CURRENT_DATE AND ((t.queue_id = q.id AND t.status IN ('escalated')) OR EXISTS (SELECT 1 FROM sla_breaches sb WHERE sb.transaction_id = t.id AND sb.fallback_queue_id = q.id AND sb.status = 'breached'))),
			total_cases = (SELECT COUNT(DISTINCT t.id) FROM transactions t WHERE t.ingested_at >= CURRENT_DATE AND (t.queue_id = q.id OR EXISTS (SELECT 1 FROM sla_breaches sb WHERE sb.transaction_id = t.id AND (sb.original_queue_id = q.id OR sb.fallback_queue_id = q.id) AND sb.breached_at >= CURRENT_DATE))),
			...
			breach_rate = CASE 
				WHEN (SELECT COUNT(DISTINCT t.id) FROM transactions t WHERE t.ingested_at >= CURRENT_DATE AND (t.queue_id = q.id OR EXISTS (SELECT 1 FROM sla_breaches sb WHERE sb.transaction_id = t.id AND sb.original_queue_id = q.id AND sb.breached_at >= CURRENT_DATE))) > 0 THEN
					((SELECT COUNT(DISTINCT t.id) FROM transactions t LEFT JOIN reviews r ON r.transaction_id = t.id WHERE t.ingested_at >= CURRENT_DATE AND ((t.queue_id = q.id AND ((r.reviewed_at IS NOT NULL AND EXTRACT(EPOCH FROM (r.reviewed_at - t.ingested_at)) / 60.0 > q.sla_target_minutes) OR (r.reviewed_at IS NULL AND EXTRACT(EPOCH FROM (NOW() - t.ingested_at)) / 60.0 > q.sla_target_minutes))) OR EXISTS (SELECT 1 FROM sla_breaches sb WHERE sb.transaction_id = t.id AND sb.original_queue_id = q.id AND sb.breached_at >= CURRENT_DATE)))::float / (SELECT COUNT(DISTINCT t.id) FROM transactions t WHERE t.ingested_at >= CURRENT_DATE AND (t.queue_id = q.id OR EXISTS (SELECT 1 FROM sla_breaches sb WHERE sb.transaction_id = t.id AND sb.original_queue_id = q.id AND sb.breached_at >= CURRENT_DATE)))::float) * 100.0
				ELSE 0.0
			END
	`

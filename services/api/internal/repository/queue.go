...
func (r *QueueRepository) List(ctx context.Context) ([]model.Queue, error) {
	updateQuery := `
		UPDATE queues q SET
			...
			open_cases = (SELECT COUNT(DISTINCT t.id) FROM transactions t WHERE t.ingested_at >= CURRENT_DATE AND t.queue_id = q.id AND (t.status = 'escalated' OR (t.status = 'reviewed' AND (SELECT decision FROM reviews r WHERE r.transaction_id = t.id ORDER BY r.reviewed_at DESC LIMIT 1) = 'escalate'))),
			cases_breached = (SELECT COUNT(DISTINCT t.id) FROM transactions t WHERE t.ingested_at >= CURRENT_DATE AND ((t.queue_id = q.id AND t.status = 'breached') OR EXISTS (SELECT 1 FROM sla_breaches sb WHERE sb.transaction_id = t.id AND sb.original_queue_id = q.id))),
			breach_rate = CASE 

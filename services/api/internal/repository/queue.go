...
func (r *QueueRepository) List(ctx context.Context) ([]model.Queue, error) {
	updateQuery := `
		UPDATE queues q SET
			open_cases = (SELECT COUNT(*) FROM transactions t WHERE t.queue_id = q.id AND t.status = 'escalated' AND t.ingested_at >= CURRENT_DATE),
			total_cases = (SELECT COUNT(*) FROM transactions t WHERE t.queue_id = q.id AND t.ingested_at >= CURRENT_DATE),
			cases_breached = (SELECT COUNT(*) FROM transactions t LEFT JOIN reviews r ON r.transaction_id = t.id WHERE t.queue_id = q.id AND t.ingested_at >= CURRENT_DATE AND ((r.reviewed_at IS NOT NULL AND EXTRACT(EPOCH FROM (r.reviewed_at - t.ingested_at)) / 60.0 > q.sla_target_minutes) OR (r.reviewed_at IS NULL AND EXTRACT(EPOCH FROM (NOW() - t.ingested_at)) / 60.0 > q.sla_target_minutes))),
			breach_rate = CASE 
				WHEN (SELECT COUNT(*) FROM transactions t WHERE t.queue_id = q.id AND t.ingested_at >= CURRENT_DATE) > 0 THEN
					((SELECT COUNT(*) FROM transactions t LEFT JOIN reviews r ON r.transaction_id = t.id WHERE t.queue_id = q.id AND t.ingested_at >= CURRENT_DATE AND ((r.reviewed_at IS NOT NULL AND EXTRACT(EPOCH FROM (r.reviewed_at - t.ingested_at)) / 60.0 > q.sla_target_minutes) OR (r.reviewed_at IS NULL AND EXTRACT(EPOCH FROM (NOW() - t.ingested_at)) / 60.0 > q.sla_target_minutes)))::float / (SELECT COUNT(*) FROM transactions t WHERE t.queue_id = q.id AND t.ingested_at >= CURRENT_DATE)::float) * 100.0
				ELSE 0.0
			END
	`

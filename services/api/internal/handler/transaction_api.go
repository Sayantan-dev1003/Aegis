...
func (h *TransactionHandler) GetByID(w http.ResponseWriter, r *http.Request) {
	...
	resp := model.TransactionDetailResponse{
		Transaction: model.TransactionDetail{
			...
			RiskSource:          tx.RiskSource,
			RejectCount:         tx.RejectCount,
			SLABreachType:       tx.SLABreachType,
			RequiresAdminReview: tx.RequiresAdminReview,
			ClaimedAt:           tx.ClaimedAt,
			ClaimedByName:       tx.ClaimedByName,
			SLAPausedAt:         tx.SLAPausedAt,
			BreachedAt:          tx.BreachedAt,
			EscalatedAt:         tx.EscalatedAt,
			EscalatedTo:         tx.EscalatedTo,
		},
	}

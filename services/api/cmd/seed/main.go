...
func main() {
	...
	logger.Get().Info().Str("path", seedScriptPath).Msg("Reading seed script...")
	content, err := os.ReadFile(seedScriptPath)
	if err != nil {
		logger.Get().Fatal().Err(err).Msg("Failed to read seed script file")
	}
	...
	logger.Get().Info().Msg("Executing seeding queries...")
	_, err = pgPool.Exec(ctx, string(content))
	if err != nil {
		logger.Get().Fatal().Err(err).Msg("Failed to execute seed queries")
	}

	logger.Get().Info().Msg("Database seeded successfully!")
}
